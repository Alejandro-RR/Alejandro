 ...
 * @return array
 */
function automatorwp_everest_forms_options_cb_form( $field ) {

    // Setup vars
    $value = $field->escaped_value;
    $none_value = 'any';
    $none_label = __( 'any form', 'automatorwp-everest-forms' );
    ...
    if( ! empty( $value ) ) {
        ...
        foreach( $value as $form_id ) {
            ...
            $options[$form_id] = automatorwp_everest_forms_get_form_title( $form_id );
        }
    }
 ...
 * @return string|null
 */
function automatorwp_everest_forms_get_form_title( $form_id ) {
    ...
    $form = wpFluent()->table( 'everest_forms_forms' )
...
function automatorwp_everest_forms_parse_automation_tags( $parsed_content, $replacements, $automation_id, $user_id, $content ) {
    ...
    foreach( $triggers as $trigger ) {
        ...
        if( $trigger_args['integration'] !== 'everest-forms' ) {
            continue;
        }
